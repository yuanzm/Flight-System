...
exports.signUp = function(req, res, next) {
    ...
    // 验证信息的完整性
    if ([loginname, password, rePassword, identitynumber].some(function(item) {return item === ''})) {
    ...
    User.getUserByQuery({
        '$or': [{'loginname': loginname}, {'identitynumber': identitynumber}]
    }, function(err, users) {
        if(err) {
            return next(err);
        }
        if (users.length) {
            return ep.emit('sign_err', '该用户名或身份证已经被使用');
        } else {
            var md5 = crypto.createHash('md5');
            var passHash = md5.update(password).digest('base64');

            User.newAndSave(loginname, passHash, identitynumber, function(err, user) {
                if (err) {
                    return next(err);
                }
                res.status(200);
